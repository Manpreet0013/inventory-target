    ...
    @forelse($product->targets->whereNull('parent_id') as $target)
        ...
        <div class="bg-white rounded shadow p-5 mb-5 border-l-4 border-blue-500 hover:shadow-lg transition">
            <div class="flex justify-between flex-wrap gap-4 items-start">
                <div>
                    <p><b>Product Admin:</b> 
                        <span class="px-2 py-1 rounded bg-red-100 text-red-800 text-xs font-semibold">{{ $target->executive->name ?? '-' }}</span>
                    </p>
                    <p><b>Target Value:</b> {{ $target->target_value }} ({{ ucfirst($target->target_type) }})</p>
                    <p><b>Status:</b> 
                        <span class="px-3 py-1 rounded-full text-xs {{ $statusClasses[$target->status] ?? 'bg-gray-200 text-gray-800' }}">
                ...
                <div class="w-64">
                    <div class="text-xs mb-1 font-medium">{{ $achieved }} achieved / {{ $remaining }} remaining</div>
                    <div class="w-full bg-gray-200 rounded h-3 overflow-hidden">
                        <div class="h-3 rounded {{ $percentage >= 100 ? 'bg-green-600' : 'bg-blue-500' }}" 
                    ...
                    @foreach($target->children as $child)
                        ...
                        <div class="bg-gray-50 rounded p-4 border-l-4 border-purple-500 hover:shadow-md transition">
                            <div class="flex justify-between flex-wrap gap-4 items-start text-sm">
                                <div>
                                    <p><b>Executive:</b> 
                                        <span class="px-2 py-1 rounded bg-indigo-500 text-white text-xs">{{ $child->executive->name ?? '-' }}</span>
                                    </p>
                                    <p><b>Target:</b> {{ $child->target_value }} ({{ ucfirst($child->target_type) }})</p>
                                    <p><b>Status:</b> 
                                        <span class="px-3 py-1 rounded-full text-xs {{ $statusClasses[$child->status] ?? 'bg-gray-200 text-gray-800' }}">
                                ...
                                <div class="w-48">
                                    <div class="text-xs mb-1 font-medium">{{ $childAchieved }} achieved / {{ $childRemaining }} remaining</div>
                                    <div class="w-full bg-gray-200 rounded h-2 overflow-hidden">
                                        <div class="h-2 rounded {{ $childPercentage >= 100 ? 'bg-green-600' : 'bg-purple-500' }}" 
                                                ...
                                                <tr class="hover:bg-gray-50">
                                                    ...
                                                    <td class="border px-2 py-1">
                                                        <span class="px-3 py-1 rounded-full text-xs
                                                            {{ $statusClasses[$sale->status] ?? 'bg-gray-200 text-gray-800' }}">
                                                            {{ ucfirst($sale->status) }}
                                                        </span>
                                                    </td>
                                                    <td class="border px-2 py-1">
                                                        <span class="px-3 py-1 rounded-full text-xs
                                                            {{ $statusClasses[$sale->accountant_status] ?? 'bg-gray-200 text-gray-800' }}">
                        ...
                        <tbody>
                            @foreach($target->sales as $pIndex => $sale)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="border px-2 py-1">{{ $sale->sale_date }}</td>
                                    <td class="border px-2 py-1">
                                        <span class="px-3 py-1 rounded-full text-xs {{ $statusClasses[$sale->status] ?? 'bg-gray-200 text-gray-800' }}">
                                            {{ ucfirst($sale->status) }}
                                        </span>
                                    </td>
                                </tr>
